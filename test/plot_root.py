...
def draw_wp_root(eff,fr,**kwargs):
    gr = r.TGraph()
...
def draw_roc_root(tpr,fpr,auc,eff=1.,fr=1.,**kwargs):
...
def draw_all_root(lowpt,egamma,eta_upper,pt_lower,pt_upper=None):
    ...
    #draw_wp_root(trk_eff,trk_fr,label='Low-pT track',mfc='black')
    ...
    draw_wp_root(gsf_eff,gsf_fr,label='Low-pT GSF track',style=20,size=2,color=r.kBlue)
    ...
    draw_wp_root(ele_eff,ele_fr,label='Low-pT electron',style=20,size=1.6,color=r.kRed)
    ...
    draw_roc_root(tpr,fpr,auc,ele_eff,ele_fr,label='Unbiased seed',color=r.kBlue)
    ...
    fpr,tpr,thr = roc_curve(lowpt.is_e[has_obj],lowpt[branch][has_obj])
    auc = roc_auc_score(lowpt.is_e[has_obj],lowpt[branch][has_obj]) if len(set(lowpt.is_e[has_obj])) > 1 else 0.
    draw_roc_root(tpr,fpr,auc,ele_eff,ele_fr,label='Biased seed',color=r.kBlue,style=2)
    ...
    fpr,tpr,thr = roc_curve(lowpt.is_e[has_obj],lowpt[branch][has_obj])
    auc = roc_auc_score(lowpt.is_e[has_obj],lowpt[branch][has_obj]) if len(set(lowpt.is_e[has_obj])) > 1 else 0.
    draw_roc_root(tpr,fpr,auc,ele_eff,ele_fr,label='ID, 2020Sept15',color=r.kRed)
    ...
    fpr,tpr,thr = roc_curve(lowpt.is_e[has_obj],lowpt[branch][has_obj])
    auc = roc_auc_score(lowpt.is_e[has_obj],lowpt[branch][has_obj]) if len(set(lowpt.is_e[has_obj])) > 1 else 0.
    draw_roc_root(tpr,fpr,auc,ele_eff,ele_fr,label='ID, 2019Aug07',color=r.kRed,style=2)
    ...
    fpr,tpr,thr = roc_curve(lowpt.is_e[has_obj],lowpt[branch][has_obj])
    auc = roc_auc_score(lowpt.is_e[has_obj],lowpt[branch][has_obj]) if len(set(lowpt.is_e[has_obj])) > 1 else 0.
    draw_roc_root(tpr,fpr,auc,ele_eff,ele_fr,label='ID, 2021May17',color=r.kRed,style=3)
    ...
    fpr,tpr,thr = roc_curve(lowpt.is_e[has_obj],lowpt[branch][has_obj])
    auc = roc_auc_score(lowpt.is_e[has_obj],lowpt[branch][has_obj]) if len(set(lowpt.is_e[has_obj])) > 1 else 0.
    draw_roc_root(tpr,fpr,auc,ele_eff,ele_fr,label='ID, 2020Nov28',color=r.kRed,style=4)
    ...
    gsf_fr = float(numer.sum()) / float(denom.sum()) if float(denom.sum()) > 0. else 0.
    draw_wp_root(gsf_eff,gsf_fr,label='Egamma',style=20,size=2,color=r.kOrange+1)
    ...
    ele_fr = float(numer.sum()) / float(denom.sum()) if float(denom.sum()) > 0. else 0.
    draw_wp_root(ele_eff,ele_fr,label='PF electron',style=20,size=1.6,color=r.kMagenta+2)
    ...
    draw_roc_root(tpr,fpr,auc,ele_eff,ele_fr,label='ID, PF',color=r.kMagenta+2,style=2)
    ...
    fpr,tpr,thr = roc_curve(egamma.is_e[has_obj],egamma[branch][has_obj])
    auc = roc_auc_score(egamma.is_e[has_obj],egamma[branch][has_obj]) if len(set(egamma.is_e[has_obj])) > 1 else 0.
    draw_roc_root(tpr,fpr,auc,ele_eff,ele_fr,label='ID, PF retrained',color=r.kMagenta+2)

################################################################################

def plot_roc_all_root(lowpt,egamma,eta_upper,pt_lower,pt_upper=None):
    ...
    draw_all_root(lowpt,egamma,eta_upper,pt_lower)
    ...
    for i,gr in enumerate(graphs):
        print(i,gr.GetName(),gr.GetTitle())
    ...
    c.SaveAs(f"roc_root_prelim.pdf")
