...
################################################################################
# 
def bparking_trk(dir,test,egamma,has_pfgsf_branches=True,AxE=True) :
   print('##### BPARKING TRK ##########################################################')
   ...
   ########################################
   # Electron (pT > 0.5 GeV, Tight WP for Seed BDT) 

   has_trk = (test.has_trk) & (test.trk_pt>0.5) & (np.abs(test.trk_eta)<2.5)
   has_ele = (test.has_ele) & (test.ele_pt>0.5) & (np.abs(test.ele_eta)<2.5) & (test.gsf_bdtout1>3.05)
   denom = has_trk&test.is_e; numer = has_ele&denom;
   ele_T_eff = float(numer.sum()) / float(denom.sum()) if float(denom.sum()) > 0. else 0.
   denom = has_trk&(~test.is_e); numer = has_ele&denom;
   ele_T_fr = float(numer.sum()) / float(denom.sum()) if float(denom.sum()) > 0. else 0.
   plt.plot([ele_T_fr], [ele_T_eff],
            marker='o', markerfacecolor='white', markeredgecolor='blue', 
            markersize=8,linestyle='none',
            label='Low-pT electron (Tight Seed)',
            )
...
#   denom = has_trk&egamma.is_e; numer = has_pfgsf&denom
...
#            label='(Reduced) EGamma GSF track')
   ...
   same_eff = test[id_branch]>id_score[id_ELE]
   ...
   for attr,binning,xlabel in zip(*tuple) :
      ...
      curves = [
         ...
         {"label":"Same mistag rate","var":test[attr],"mask":(test.is_e)&(has_trk),"condition":(has_ele)&(same_fr),"colour":"blue","fill":False,"size":8,"marker":"*"},
         {"label":"Low-pT electron (Tight Seed)","var":test[attr],"mask":(test.is_e)&(has_trk),"condition":(has_ele_T),"colour":"blue","fill":False,"size":8,},
