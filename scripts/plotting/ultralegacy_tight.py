...
################################################################################
# 
def ultralegacy_tight(dir,test,egamma,has_pfgsf_branches=True,AxE=True) :
   print('##### ULTRA LEGACY TIGHT ##########################################################')
   ...
   has_gsf = (test.has_gsf) & (test.gsf_pt>0.5) & (np.abs(test.gsf_eta)<2.5) & \
       (test.gsf_pt>1.0) & ( (test.gsf_bdtout1>3.05) | (test.gsf_bdtout2>2.42) )
   ...
   has_gsf = (egamma.has_gsf) & (egamma.gsf_pt>0.5) & (np.abs(egamma.gsf_eta)<2.5) & \
       (egamma.gsf_pt>1.0) & ( (egamma.gsf_bdtout1>3.05) | (egamma.gsf_bdtout2>2.42) )
   ...
   for attr,binning,xlabel in zip(*tuple) :
      ...
      curves = [
         {"label":"UltraLegacy AOD","var":test[attr],"mask":(test.is_e)&(has_gsf),"condition":(has_ele),"colour":"blue","fill":True,"size":8,},
         {"label":"UL MINIAOD?","var":test[attr],"mask":(test.is_e)&(has_gsf),"condition":(has_ele)&(double_fr),"colour":"blue","fill":False,"size":8,"marker":"^"},
         {"label":"UL MINIAOD?","var":test[attr],"mask":(test.is_e)&(has_gsf),"condition":(has_ele)&(same_fr),"colour":"blue","fill":False,"size":8,"marker":"v"},
