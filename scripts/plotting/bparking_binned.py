...
def bparking_binned(dir,test,egamma,has_pfgsf_branches=True,AxE=True) :
   ...
   has_trk = (test.has_trk) & (test.trk_pt>0.5) & (np.abs(test.trk_eta)<2.5)
   has_ele = (test.has_ele) & (test.ele_pt>0.5) & (np.abs(test.ele_eta)<2.5) & (test.ele_pt>2.0)
   denom = has_trk&test.is_e; numer = has_ele&denom;
   ...
   plt.plot([ele_high_fr], [ele_high_eff],
   ...
   id_high_fpr,id_high_tpr,id_high_score = roc_curve(test.is_e[has_ele],test[id_high_branch][has_ele])
   id_high_auc = roc_auc_score(test.is_e[has_ele],test[id_high_branch][has_ele]) if len(set(test.is_e[has_ele])) > 1 else 0.
   plt.plot(id_high_fpr*ele_high_fr, 
   ...
   plt.plot([ele_low_fr], [ele_low_eff],
   ...
   id_low_fpr,id_low_tpr,id_low_score = roc_curve(test.is_e[has_ele],test[id_low_branch][has_ele])
   id_low_auc = roc_auc_score(test.is_e[has_ele],test[id_low_branch][has_ele]) if len(set(test.is_e[has_ele])) > 1 else 0.
   ...
   ##########
   # Finish up ... 
   plt.legend(loc='upper left',facecolor='white',framealpha=None,frameon=False)
   plt.tight_layout()
