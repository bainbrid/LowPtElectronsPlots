...
################################################################################
# 
def bparking_dev3(dir,test,egamma,has_pfgsf_branches=True,AxE=True) :
   print('##### BPARKING DEV3 ##########################################################')
...
#         plt.text(x, y+0.02, f"{threshold:.1f}", fontsize=6, ha='center', va='center', color='green' )
   ...
   ########################################
   # Electron (1.5 < pT < 2.0 GeV)

   has_med = has_ele & (test.trk_pt>1.5) & (test.trk_pt<2.0)
   ...
   ########################################
   # Electron (1.0 < pT < 1.5 GeV)

   has_low = has_ele & (test.trk_pt>1.0) & (test.trk_pt<1.5)
   ...
   if TRK_DENOM :
      ...
      print(len(pf_has_trk),pf_has_trk.sum())
      print(pf_has_trk)

      pf_has_high = pf_has_ele & (egamma.trk_pt>2.0)
      ...
      pf_has_med = pf_has_ele & (egamma.trk_pt>1.5) & (egamma.trk_pt<2.0)
      ...
      pf_has_low = pf_has_ele & (egamma.trk_pt>1.0) & (egamma.trk_pt<1.5)
   ...
   ##########
   # Finish up ... 
   plt.legend(loc='center right',facecolor='white',framealpha=None,frameon=False) # 'center right'
   plt.tight_layout()
