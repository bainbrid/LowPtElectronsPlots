...
################################################################################
# 
def bparking_dev5(dir,test,egamma,has_pfgsf_branches=True,AxE=True) :
   print('##### BPARKING DEV5 ##########################################################')
   ...
   plt.xlim(1.e-5,1.)
   plt.ylim(0.,1.0 if inclusive else 0.35)
   ...
   if inclusive :
      ...
      fr = float(numer.sum()) / float(denom.sum()) if float(denom.sum()) > 0. else 0.
      plt.plot([fr], [eff],
               marker='s', markerfacecolor='blue', markeredgecolor='blue', 
               markersize=8,linestyle='none',
      ...
      if TRK_DENOM :
         ...
         fr = float(numer.sum()) / float(denom.sum()) if float(denom.sum()) > 0. else 0.
         plt.plot([fr], [eff],
                  marker='s', markerfacecolor='green', markeredgecolor='green', 
                  markersize=8,linestyle='none',
